...
function loadModel(data,resourceName) 
	if data and data.flag ~= "SA_PROP" then
		...
		if id ~= false then -- do it on id is avaiable
			...
			if engineImportTXD(texture,id,true) then system.loadedTxd = system.loadedTxd + 1 end

			-- load dff
			path = ':'..resourceName..'/Content/models/'..data.model..'.dff'
			local model = requestModel(path)
			if engineReplaceModel(model,id,isTransparentFlag(data.flag)) then system.loadedModel = system.loadedModel + 1 end
			

			-- load col
			path = ':'..resourceName..'/Content/coll/'..data.collision..'.col'
			local collision = requestCollision(path)
			if engineReplaceCOL(collision,id) then system.loadedCol = system.loadedCol + 1 end
			
			-- check if is lods
			if string.find(data.model,"lod") then
				mapdata[resourceName].lods[data.model] = {
					id = id,
					model = data.model,
				}
			end
...
function getLODInfo(map,lodname,x,y) 
	--if mapdata[map].lods[lodname] then 
		--return mapdata[map].lods[lodname]
	--end
		...
		if lod or tonumber(data.info.draw) >= 1000 or FORCE_LODS then
			...
			if flag ~= "SA_PROP" then
				if USE_LODS then -- do it when it enabled
					...
					local lodinfo = getLODInfo(mapname,lod,x,y) 
					if lodinfo then
...
function loadMap(ipls,ides,offset,mapname) 
	...
	Async:forkey(ides, function(key,data) 
		...
		if loaded >= total then 
			...
			outputChatBox (string.format("[Streamer]: Total %s objs",total))
...
addCommandHandler ( "loadedinfo", function() 
	iprint(system)
end)
addCommandHandler ( "mapinfo", function(cmd,map) 
	iprint({
		ipls = #mapdata[map].ipls,
		ides = #mapdata[map].ides,
	})
end)