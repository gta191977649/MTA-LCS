...
for i,v in pairs(IPLTable) do
	...
	-- cache lods in a hash loopup table
	if Version == "VC" or Version == "LCS" then 
		for iA,vA in ipairs(Proccessed_tmp) do
			if vA[2] and tonumber(vA[1]) and tonumber(vA[3]) and tonumber(vA[4]) and (not tonumber(vA[2])) then
				local dff = removeSpace(vA[2])
				if string.match(dff, "^LOD.*") then
					LodExists[string.lower(dff)] = true
	...
	for iA,vA in ipairs(Proccessed_tmp) do
		if Version == "SA" or Version == "VCS" then -- new renderware use Lod linking system
			if vA[2] and tonumber(vA[1]) and tonumber(vA[3]) and tonumber(vA[4]) and (not tonumber(vA[2])) then
		...
		else -- otherwise, use The LOD models have the same name as the original model, except the first three letters of the LOD model are replaced by the letters "LOD." Create an LOD version of the collision model of the original model. The LOD collision should be the same size as the original model's but with nothing in it.
			if vA[2] and tonumber(vA[1]) and tonumber(vA[3]) and tonumber(vA[4]) and (not tonumber(vA[2])) then
				local dff = removeSpace(vA[2])
				Exists[dff] = true
				if not string.match(dff, "^LOD.*") then
					-- Replaced first 3 letters to LOD (GTA VC, III, LCS) engine definiation
					LOD_NAME = string.gsub(dff, "^...(.*)", "LOD%1")
					-- find lods 
					LODS[dff] = LodExists[string.lower(LOD_NAME)] and LOD_NAME or -1
				else -- if is lods ide, thus no lods contains
					LODS[dff] = -1
				end
			end
		end
